...
Keeps the rest of src/ free of path juggling: it resolves the model-slot root and
the repo root, loads the YAML configs, and imports `_infra/` utilities by file
path (so models genuinely reuse the shared platform without needing `_infra` to
be an installed package).

This slot also ships standalone, so `_find_repo_root` returns None rather than
raising when there is no `_infra/` above it; `infra()` raises only if a
shared-platform module is actually requested. `HAS_INFRA` tells you which mode
you are in.
"""
...
def infra(rel_module_path: str):
    ...
    if REPO_ROOT is None:
        raise ImportError(
            ...
            f"above {SLOT_ROOT} — this slot is running standalone."
        )
...
def resolve_path(rel: str) -> str:
    """Resolve a slot-relative path (e.g. an output dir) to absolute."""
    p = Path(rel)
