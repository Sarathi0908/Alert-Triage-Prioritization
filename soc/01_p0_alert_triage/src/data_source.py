...
data/build_ids2018_dataset.py (identity / ephemeral / random columns already
dropped at build time) and is gitignored — regenerate it, never commit it.
...
# A few NetFlow rate columns exceed float32 range; clip rather than overflow to inf.
_F32_CLIP = 3e38


@dataclass
class FlowDataset:
    """What the pipeline needs from one ingest: the table, its label columns and
    the manifest that describes how it was built."""
    frame: pd.DataFrame
    features: list[str]
    target: str
...
def load_selected(model_cfg: dict) -> dict:
    """The screened model feature set written by the feature screener."""
...
def load_dataset(model_cfg: dict, columns: list[str] | None = None,
                 log=print) -> FlowDataset:
    """Ingest entry point used by src/pipeline.py."""
    manifest = load_manifest(model_cfg)
    frame = load_frame(model_cfg, columns=columns)
    target, cls = model_cfg["target"], model_cfg["attack_class_column"]
    feats = list(columns) if columns else [
        c for c in frame.columns if c not in (target, cls)]
    ds = FlowDataset(frame=frame, features=feats, target=target,
                     attack_class_col=cls, manifest=manifest)
    log(f"dataset={manifest.get('dataset')}  rows={ds.n_rows:,}  "
        f"attack={ds.attack_rate * 100:.3f}%  features={len(feats)}")
    return ds
...
def subsample(df: pd.DataFrame, n: int, seed: int) -> pd.DataFrame:
    ...
    permutation rather than the original row order. That is deliberate: the
    screening ranker runs without LightGBM's `deterministic` / `force_row_wise`
    flags, so its threaded histogram sums are mildly order-sensitive and
    short-circuiting here shifts the reported gain percentages by ~0.1pp.
    """
